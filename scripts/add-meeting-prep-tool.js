...
const allContacts = (await httpGet('https://api.crmworkspace.com/v1/contacts?per_page=250', { 'ACCESS_TOKEN': process.env.WEALTHBOX_TOKEN })).contacts || [];
...
const notesData = await httpGet('https://api.crmworkspace.com/v1/notes?contact_id=' + contact.id + '&per_page=5', { 'ACCESS_TOKEN': process.env.WEALTHBOX_TOKEN });
...
const tokenData = await httpPost('https://login.microsoftonline.com/' + process.env.GRAPH_TENANT_ID + '/oauth2/v2.0/token', null, {});
// Use form-encoded for token
const tokenBody = 'client_id=' + process.env.GRAPH_CLIENT_ID + '&client_secret=' + encodeURIComponent(process.env.GRAPH_CLIENT_SECRET) + '&scope=https://graph.microsoft.com/.default&grant_type=client_credentials';
const tokenRes = await new Promise((resolve) => {
  const req = https.request({ hostname: 'login.microsoftonline.com', path: '/' + process.env.GRAPH_TENANT_ID + '/oauth2/v2.0/token', method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Content-Length': Buffer.byteLength(tokenBody) } }, (res) => {
    let d = ''; res.on('data', c => d += c);
    res.on('end', () => { try { resolve(JSON.parse(d)); } catch { resolve({}); } });
  });
  req.on('error', () => resolve({}));
...
await httpPost('https://api.crmworkspace.com/v1/notes', { content: auditNote, linked_to: [{ id: contact.id, type: 'Contact' }], tags: [{ name: 'DAX' }, { name: 'Automated' }] }, { 'ACCESS_TOKEN': process.env.WEALTHBOX_TOKEN });
