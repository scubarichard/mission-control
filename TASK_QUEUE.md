...
### Completion Notes
✅ All 3 secrets stored in `kvdaxdakonapilot`:
- `google-oauth-1altx-client-id`
- `google-oauth-1altx-client-secret`
- `google-oauth-1altx-refresh-token`

✅ Token verified valid — refresh_token → access_token exchange succeeded

⚠️ **SCOPE GAP for Atlas:** The existing OAuth consent only covers **Gmail** scopes, NOT Calendar API.
...
- **For calendar access**: Richard must re-authorize the OAuth app with Calendar scope added (`https://www.googleapis.com/auth/calendar`). Until then, Atlas can only read Gmail for richard@1altx.com — not calendar events.

**Extraction method:** Azure VM run-command → SQLite query → AES-256-CBC decrypt with n8n encryption key from `/home/dkn8n/.n8n/config`
...
- **From:** Atlas
- **Client:** Atlas / Infrastructure
- **Deliverable:** Google OAuth tokens in Key Vault for both Gmail accounts

### Requirements
1. **richard@1altx.com** — Google OAuth token
   - Visit: https://console.cloud.google.com
   - Create OAuth 2.0 Client ID (Desktop)
   - Grant Calendar API + Gmail API scopes
   - Store in Key Vault:
     - `google-oauth-1altx-client-id`
     - `google-oauth-1altx-client-secret`
     - `google-oauth-1altx-refresh-token` (preferred, or run OAuth flow)

2. **richard@mabbun.com** — Google OAuth token
   - Same process, store as `google-oauth-mabbun-*`

### Why This Matters
Atlas currently has access to M365/Outlook (rmabbun@dakona.com) but cannot read Gmail calendars or emails. Once these tokens exist, daily briefings will include events from all three calendars + inbox digests.

### Handoff
Once tokens are in the vault, Atlas will:
- Deploy n8n workflows for calendar fetch
- Deploy n8n workflows for email digest
- Wire into morning briefing + EOD recap
...
- ⚠️ YouTube upload limit is a **rolling 24-hour window**, NOT a midnight PDT reset
    - 02:05am CDT May 3: ALL 19 FAILED (limit not reset)
    - 10:05am CDT May 3: ALL 19 FAILED (still within 24hr window from ~22:30 CDT May 2 uploads)
    - Last successful uploads were ~22:30-23:00 CDT May 2 → window clears ~22:30-23:00 CDT May 3
- ⏳ **Retry** (23:30 CDT May 3): `CatalogMint-YT-Upload` rescheduled → window guaranteed clear by then (19 videos: 11-19, 21-30)
- ⏳ **Night 2** (03:00 CT May 4): `CatalogMint-YT-Upload-D2` still set → handles any remaining after May 3 retry
...
- **Assignee:** Forge
- **Status:** DONE
- **Completed:** 2026-05-01 23:51 UTC
- **Completed_by:** Forge (Access Policies method)
- **Notes:** ✅ Atlas can now read all 68 secrets in kvdaxdakonapilot. Verified via `az keyvault secret list`.
- **Priority:** HIGH
- **From:** Richard (Telegram 2026-05-01 20:54 UTC)
- **Client:** Atlas / Infrastructure

### Completion Notes
- First attempt (RBAC role) failed because vault uses Access Policies
- Forge corrected approach and added SP to Access Policies
- Atlas verified access works at 2026-05-01 23:51 UTC
...
**What to do:**
1. Go to **Azure Portal** → Search `kvdaxdakonapilot`
2. Click the vault → **Access Control (IAM)** on the left
3. Click **+ Add** → **Add role assignment**
4. Role: `Key Vault Secrets Officer`
5. Assign to: Service principal → Search `9ddd0bc6-4ff2-4579-9292-bce677a06fd1`
6. Click **Save**
...
## TASK-20260429-NAUTILUS-002 — Copilot Studio Migration Setup
- **Assignee:** Nautilus
- **Status:** PENDING
- **Priority:** High
- **From:** Richard (dax-collab Slack 2026-04-29)
- **Client:** Dakona / DAX

### Task
Clone dax repo and execute Copilot Studio Migration setup per task doc.

**Repo:** https://github.com/scubarichard/dax.git
**Task doc (read after clone):** `docs/tasks/TASK-20260429-002-Copilot-Studio-Migration.md`

### Nautilus Steps
1. Clone repo, read full task doc at `docs/tasks/TASK-20260429-002-Copilot-Studio-Migration.md`
2. Create new dirs: `/repo/frontend/dev/` and `/repo/power-platform/solutions/`
3. Install dotnet + pac CLI:
   ```
   ...
   pac --version
   ```
4. After Richard caches az login — run: `pac auth create --tenant d2a3c346-00f3-47dd-a53e-caa3fca74714`
5. Signal ready in task queue → Richard adds Cloudflare CNAME (`dev.dax.dakona.com` → Azure Static Web App default domain)

### PETA Split (Richard does these — browser/interactive)
- `az login --tenant d2a3c346-00f3-47dd-a53e-caa3fca74714`
- `gh auth login`
- `pac auth create` browser completion
- Cloudflare CNAME (when Nautilus signals ready)

### Notes
- DAX system prompt committing to `docs/DAX-system-prompt-v68.md`
- Tenant: `d2a3c346-00f3-47dd-a53e-caa3fca74714`

---

## TASK-20260429-FORGE-DAX-001 — DAX Graph SDK Migration
- **Assignee:** Forge
- **Status:** DONE
...
### Task
Migrate 6 silently broken DAX tools on `dax.dakona.com` from raw `https` + hardcoded credentials to `node-fetch` + `$env.*` pattern. T6 must pass 7/7 after completion.

### Completed
- Patched 6 toolCode nodes: `tool-email-001`, `tool-calendar-001`, `tool-send-email-001`, `tool-manage-cal-001`, `tool-create-doc-001`, `sp-browser-tool-001` — all using `node-fetch` + `$env.GRAPH_*` credentials
- Fixed `ADVISOR_EMAIL` cross-contamination: `vince@dakona.com` → `rmabbun@dakona.com` in `ecosystem.config.js`, pm2 reloaded
- Patched `Market Summary Tool` (mst-1774465121362): replaced `require("https")` sequential-await loop with `node-fetch` + `Promise.all` (fixes task runner crash)
- Fixed `Flag Post to Wealthbox` (expired token causing execution abort): rewired `Flag IF` branch 0 to fire Respond + Flag Post in parallel; removed Flag Post → Respond dependency; set `onError: continueRegularOutput`
- Deleted temp test workflows: `BQc3NlZWPdT4BmUm`, `aZnJC49aLbPQLNTh`
- T6 result: **7/7 PASS** (all tests confirmed individually with SSE stream verification)

### Notes
- Wealthbox token `2565bf3734934e0facbe77c7c2accd40` is expired — Flag Post will continue failing silently until token is rotated; compliance logging is non-blocking
